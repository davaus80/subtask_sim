...
EXPECTED_SHUFFLE_COUNT=${2:-20}
EXPECTED_ROW_COUNT=${3:-30}
...
find "$EXPERIMENT_GROUP_PATH" -type d -name "shuffles" | while read -r SHUFFLE_DIR; do
  ...
  for SHUFFLE_SUBDIR in "$SHUFFLE_DIR"/*; do
    if [ -d "$SHUFFLE_SUBDIR" ]; then
      SHUFFLE_COMPLETE=0
      # Check every jsonl file in the shuffle subdirectory. A shuffle is complete if
      # any file either:
      #   (a) has at least EXPECTED_ROW_COUNT rows, OR
      #   (b) contains multiple unique values for "action_selected_name" (scalesweep
      #       runs where the model has already explored multiple arms are informative
      #       even if the row count is low)
      while IFS= read -r JSONL_FILE; do
        ...
        # Check for multiple unique action_selected_name values
        UNIQUE_ACTIONS=$(jq -r '.action_selected_name' "$JSONL_FILE" 2>/dev/null | sort -u | wc -l)
        if [ "$UNIQUE_ACTIONS" -ge 2 ]; then
          SHUFFLE_COMPLETE=1
          break
        fi
